...
MCMC <- function(Nsim){
  ...
  X[1] <- 0.5 # chute inicial
  
  for (i in 2:Nsim){
    xprop <- runif(1,0,5) # cadidato proposto com distr Uniforme
...
message("Implentando um gerador MCMC com núcleo Uniforme, temos que o valor da integral é: ", round(valor_est,5))
#--------------------------------------------------------------------#
...
hist(teste, breaks = 50, main = "Metropolis Kernel Uniforme", las = 1, fre = F, xlim = c(0,8), xlab = "Valores gerados pelo MCMC", ylab = "Densidade", col="grey")
